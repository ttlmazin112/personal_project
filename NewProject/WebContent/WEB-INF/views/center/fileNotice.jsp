...
<!-- 게시판 -->
<article>
<h1>File Notice[전체 글 개수 : <%=allRowCount %>]</h1>
...
if (allRowCount > 0) {
	for (Board board : list) {
		%>
		<tr onclick="location.href='boardFileDetail.do?num=<%=board.getNum() %>&pageNum=<%=pageNum%>'">
			<td><%=board.getNum() %></td>
			<td class="left">
			<% 
...
if (allRowCount > 0) {
	if (startPage > pageBlockSize) { // 이전 블록이 존재하는지 확인
		%> <a href="boardFileList.do?pageNum=<%=startPage - pageBlockSize %>&search=<%=search%>">[이전]</a> <%
	}
	for (int i=startPage; i<=endPage; i++) {
		if (i == pageNum) {
			%> <a href="boardFileList.do?pageNum=<%=i %>&search=<%=search%>"><span style="color: blue; font-weight: bold;">[<%=i %>]</span></a>	<%
		} else {
			%> <a href="boardFileList.do?pageNum=<%=i %>&search=<%=search%>">[<%=i %>]</a>	<%
		}
	}
	if (endPage < maxPage) { // 다음 블록이 존재하는지 확인
		%> <a href="boardFileList.do?pageNum=<%=startPage + pageBlockSize %>&search=<%=search%>">[다음]</a> <%
	}
}
%>
</div>
